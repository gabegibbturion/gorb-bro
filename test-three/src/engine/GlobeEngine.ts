...
export class GlobeEngine {
  ...
  private animate(): void {
    ...
    this.currentTime = new Date(
      this.currentTime.getTime() + deltaTime * this.timeMultiplier * 1000
    );
  ...
  private updateSunPosition(): void {
    ...
    const x =
      distance * Math.cos(sunPosition.altitude) * Math.sin(sunPosition.azimuth);
    const y = distance * Math.sin(sunPosition.altitude);
    const z =
      distance * Math.cos(sunPosition.altitude) * Math.cos(sunPosition.azimuth);
  ...
  public onSatelliteUpdateCallback(
    callback: (satellites: SatelliteEntity[]) => void
  ): void {
    this.onSatelliteUpdate = callback;
  }

  public onEntitySelectedCallback(
    callback: (entity: SatelliteEntity | null) => void
  ): void {
