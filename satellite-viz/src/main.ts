...
async function loadSatelliteData() {
  ...
  const sprite = loader.load('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjQiIGhlaWdodD0iMjQiIHZpZXdCb3g9IjAgMCAyNCAyNCIgZmlsbD0ibm9uZSIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj4KPGNpcmNsZSBjeD0iMTIiIGN5PSIxMiIgcj0iMyIgZmlsbD0iI2ZmOTI0OCIvPgo8L3N2Zz4K');
...
function generateTLEFromCOEs(name: string, a: number, e: number, i: number, omega: number, w: number, M: number): {
  name: string;
  line1: string;
  line2: string;
} {
  ...
  const line2 = `2 ${satNum.toString().padStart(5, '0')} ${i_deg.toFixed(8).padStart(8, '0')} ${omega_deg.toFixed(8).padStart(8, '0')} ${(e * 10000000).toFixed(0).padStart(7, '0')} ${w_deg.toFixed(8).padStart(8, '0')} ${M_deg.toFixed(8).padStart(8, '0')} ${n_deg.toFixed(8).padStart(11, '0')} 00000`;

  return { name, line1, line2 };
}

// Update satellite position using satellite.js
function updateSatelliteJSPosition(index: number) {
  ...
  try {
    ...
    if (positionAndVelocity && positionAndVelocity.position && positionAndVelocity.velocity) {
      ...
      // Update state
      allSatState[index] = [x, y, z, vx, vy, vz];
    }
  } catch (error) {
...
function animate() {
  ...
  for (let j = 0; j < satN; j++) {
    if (useSatelliteJS && satelliteData[j]) {
      // Use satellite.js propagation
      updateSatelliteJSPosition(j);
    } else {
      // Use custom RK2 propagation
...
async function addSatellites() {
  ...
  try {
    ...
    if (useSatelliteJS) {
      satelliteData.push(...newSatellites.map(sat => ({
        name: sat.name,
        tle: (sat as any).tle,
        state: sat.state,
        period: sat.period
...
async function loadFromTurionAPI() {
  ...
  try {
    ...
    // Store satellite data for satellite.js
    satelliteData.push(...satellites.map(sat => ({
      name: sat.name,
      tle: sat.tle,
      state: sat.state,
      period: sat.period
...
async function loadTLEs(count: number) {
  ...
  try {
    ...
    buttons.forEach(btn => {
      btn.disabled = false;
      btn.textContent = btn.id === 'load-100-tles' ? '100 TLEs' :
        btn.id === 'load-1000-tles' ? '1000 TLEs' : 'All TLEs';
    });
  }
}
